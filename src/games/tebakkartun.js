const axios = require('axios'); // Meskipun data hardcoded, pertahankan jika ada rencana fetch

module.exports = function(app) {
    // Data kartun langsung di dalam kode
    const src = [
      {
        name: "Tom and Jerry",
    ...
    // Fungsi scrape sekarang hanya mengambil data acak dari array
    async function scrape() {
        try {
            if (!Array.isArray(src) || src.length === 0) {
                throw new Error("Data sumber Tebak Kartun kosong");
            }
            return src[Math.floor(Math.random() * src.length)];
        } catch (error) {
            console.error("Error selecting cartoon:", error.message);
    ...
    const handleRequest = async (req, res) => {
        try {
            ...
            const result = await scrape(); // Tetap async untuk konsistensi
            res.json({ status: true, result: result });
        } catch (error) {
            console.error(`[tebakkartun ${req.method}] Error:`, error.message);
            res.status(500).send(`Error: ${error.message}`);
        }
    };

    // --- PATH DIUBAH DI SINI ---
    app.get('/games/tebakkartun', handleRequest);
    app.post('/games/tebakkartun', handleRequest);
