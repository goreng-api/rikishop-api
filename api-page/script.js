...
// --- [PERBAIKAN] Fungsi Inisialisasi Halaman API Utama ---
async function initializeMainApiPage() {
    ...
    const applyTheme = (theme) => { 
        ...
        if (theme === "dark") {
            ...
            body.style.background = 'linear-gradient(135deg, var(--darker), var(--dark))';
        } else {
    ...
    if (themeToggleBtn) { 
        themeToggleBtn.addEventListener("click", () => applyTheme(body.getAttribute("data-theme") === "dark" ? "light" : "dark")); 
    }
    ...
    const musicFab = document.getElementById('musicFabContainer'); 
    const fabToggle = document.getElementById('fabToggleBtn'); 
    const musicControls = document.getElementById('musicControlsExpanded'); 
    ...
    const tryStartMusic = () => { 
        if (bgMusic && !musicStarted) { 
            bgMusic.volume = parseFloat(localStorage.getItem('musicVolume') || volume?.value || 1); 
            ...
            if (volume) volume.value = bgMusic.volume; 
            bgMusic.play().then(() => { 
                ...
                if(musicFab) musicFab.style.display = 'block'; 
                musicStarted = true; 
                console.warn("Autoplay musik gagal:", e.message); 
            }); 
        } 
    }; 
    ...
    if (continueBtn && donationModal) {
        continueBtn.addEventListener("click", () => {
            donationModal.classList.remove("active");
        });
    }
    if (fabToggle && musicControls) {
        fabToggle.addEventListener('click', () => {
            if (!musicStarted) tryStartMusic(); 
            const isExpanded = musicControls.classList.toggle('active');
            fabToggle.innerHTML = isExpanded ? '<i class="fas fa-times"></i>' : '<i class="fas fa-music"></i>';
        });
    } 
    if (playPause && bgMusic) {
        playPause.addEventListener('click', () => {
            if (!musicStarted) {
                musicStarted = true; 
                if(musicFab) musicFab.style.display = 'block'; 
            }
            if (bgMusic.paused) { bgMusic.play().catch(console.error); } 
            else { bgMusic.pause(); }
        });
    } 
    if (muteBtn && bgMusic) { 
        muteBtn.addEventListener('click', () => { 
            if (musicStarted) { 
                ...
                if (!bgMusic.muted && bgMusic.volume === 0 && volume) { volume.value = 0.1; bgMusic.volume = 0.1; localStorage.setItem('musicVolume', 0.1); }
            } else {
    ...
    if (volume && bgMusic) { 
        volume.addEventListener('input', (e) => { 
            ...
            bgMusic.volume = nv; 
            localStorage.setItem('musicVolume', nv); 
            if (nv === 0 && !bgMusic.muted) { bgMusic.muted = true; if(muteBtn) muteBtn.innerHTML = '<i class="fas fa-volume-mute"></i>'; localStorage.setItem('musicMuted', true); } 
            else if (nv > 0 && bgMusic.muted) { bgMusic.muted = false; if(muteBtn) muteBtn.innerHTML = '<i class="fas fa-volume-up"></i>'; localStorage.setItem('musicMuted', false); } 
        }); 
    } 
    if (bgMusic && playPause) { 
        bgMusic.onplay = () => playPause.innerHTML = '<i class="fas fa-pause"></i>'; 
        bgMusic.onpause = () => playPause.innerHTML = '<i class="fas fa-play"></i>'; 
        bgMusic.onended = () => playPause.innerHTML = '<i class="fas fa-play"></i>'; 
    }
    if (musicFab) musicFab.style.display = 'block'; 
    ...
    try { 
        ...
        // [PERBAIKAN BUG STUCK] PASTI sembunyikan preloader
        if (apiPreloader) {
            setTimeout(() => { apiPreloader.classList.add('hidden'); }, 200); 
        }
    }
    ...
    const renderAPIs = (category = 'all') => { 
        ...
        requestAnimationFrame(() => { 
            eps.forEach((api, index) => { 
                ...
                let labelHtml = '';
                let isLabelExpired = false;
                if (api.dateAdded) {
                    try {
                        if ((Date.now() - new Date(api.dateAdded).getTime()) / 36e5 > 24) {
                            isLabelExpired = true;
                        }
                    } catch(e) { isLabelExpired = true; }
                } else {
                    isLabelExpired = true; 
                }
                ...
                if (!isLabelExpired && (apiLabel === 'NEW' || apiLabel === 'FIX')) {
                    let labelStyle = '';
                    if (apiLabel === 'FIX') {
                        labelStyle = 'background-color: var(--success); animation: pulse-success 1.5s infinite;';
                    }
                    labelHtml = `<span class="new-label" style="${labelStyle}">${apiLabel}</span>`;
                }
                ...
                card.innerHTML = `<div class="api-info"><h3><span class="api-name-text">${api.name}</span> ${labelHtml}</h3><p>${api.desc || ''}</p><span class="status ${sC}" data-path="${basePath}">${liveStatus}</span></div><button class="play-button" data-endpoint="${api.path}" data-apiname="${api.name}" data-apidesc="${api.desc||''}"><i class="fa-solid fa-play"></i> Try it now</button>`; 
    ...
    if (tabsContainer && apiList && Object.keys(apis).length > 0) { 
        ...
        Object.keys(apis).forEach(k => { 
            ...
            btn.className = "tab"; 
            btn.dataset.tab = k; 
            btn.innerText = k[0].toUpperCase() + k.slice(1); 
            tabsContainer.appendChild(btn); 
        }); 
        renderAPIs("all"); 
    } else if (apiList) { 
        apiList.innerHTML = "<p style='color: var(--gray); text-align: center; grid-column: 1 / -1;'>No endpoints found.</p>"; 
    }

    // 5. Pasang sisa event listener
    const openApiModal = (playBtn) => {
        ...
        const cleanDesc = apiDesc.replace(selectRegex, '').trim();
        ...
        respEl.innerHTML = ""; respEl.classList.add("d-none");
        loadEl.style.display = "none";
        form.innerHTML = ""; form.style.display = "block";
        submitBtn.style.display = "none";
        copyRespBtn.classList.add("d-none");
        retryBtn.classList.add('d-none'); retryBtn.removeAttribute('data-action'); 
        ...
        if (endpointPath === '/api/blacklist-info') {
            ...
            if (queryParams.length > 0) {
                ...
                queryParams.forEach(p => {
                    ...
                    if (selectConfigs[key.toLowerCase()]) {
                        const options = selectConfigs[key.toLowerCase()];
                        let optionsHtml = options.map(opt => `<option value="${opt}" ${opt === defaultValue ? 'selected' : ''}>${opt.toUpperCase()}</option>`).join('');
                        form.innerHTML += `<label for="${key}">${label} (Pilih Opsi)</label><select id="${key}" name="${key}" style="width: 100%; padding: 12px 14px; border-radius: 8px; border: 1px solid var(--input-border); background: var(--input-bg); color: var(--light); margin-bottom: 1rem; font-size: 0.95rem; font-family: inherit; transition: var(--transition);">${optionsHtml}</select>`;
                    } else {
            ...
            if (hasQueryParams) { submitBtn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Submit'; submitBtn.style.display = "flex"; } 
            else { submitBtn.innerHTML = '<i class="fa-solid fa-play"></i> Try it'; submitBtn.style.display = "flex"; }
    ...
    async function fetchAPI(url) {
        ...
        loadEl.style.display = "block"; respEl.classList.add("d-none"); respEl.className = 'd-none'; 
    ...
    async function fetchAndDisplayBlacklistInfo() {
        ...
        respEl.classList.add("d-none"); respEl.innerHTML = ''; respEl.className = ''; respEl.classList.add('d-none', 'blacklist-info'); 
        try {
    ...
    const mainApiPageEl = document.getElementById('main-api-page');
    if (mainApiPageEl) {
        mainApiPageEl.addEventListener("click", (e) => {
            const playBtn = e.target.closest(".play-button:not(.btn-secondary):not(#continueBtn):not(#retryRequestBtn):not(#docs-page-button)"); if (playBtn && playBtn.closest('.api-card')) openApiModal(playBtn);
            ...
            if (retryBtn) {
                if (retryBtn.getAttribute('data-action') === 'fetchBlacklist') { fetchAndDisplayBlacklistInfo(); } 
                else {
    ...
    function submitParams() { const form = document.getElementById("paramForm"); if(!form) return; const akInp = form.querySelector('input[name="apikey"]'); if (currentEndpointRequiresApiKey && akInp && !akInp.value.trim()) { showNotification("API Key required!", 'error'); akInp.focus(); return; } if (form.innerHTML && !form.checkValidity()) { const inv = form.querySelector(':invalid'); if (inv) inv.focus(); return; } form.style.display = "none"; const sBtn = document.getElementById("submitParamBtn"); if(sBtn) sBtn.style.display = "none"; const rEl = document.getElementById("apiResponseContent"); if(rEl) {rEl.innerHTML = ""; rEl.classList.add("d-none");} const cpBtn = document.getElementById("copyResponseBtn"); if(cpBtn) cpBtn.classList.add("d-none"); const ldEl = document.getElementById("apiResponseLoading"); if(ldEl) ldEl.style.display = "block"; const rtBtn = document.getElementById("retryRequestBtn"); if(rtBtn) rtBtn.classList.add('d-none'); let finalURL; const basePath = currentEndpointPath.split("?")[0]; if (form.innerHTML) { const inputs = form.querySelectorAll("input, select"); const query = Array.from(inputs).filter(i=>i.value.trim()||i.required).map(i => `${encodeURIComponent(i.name)}=${encodeURIComponent(i.value)}`).join("&"); finalURL = `${basePath}${query?'?'+query:''}`; } else { finalURL = basePath; } lastFetchedUrl = finalURL; fetchAPI(finalURL); }
...
if (reportForm) { 
    reportForm.addEventListener("submit", async (e) => { 
        ...
        if (imageFile && imageFile.size > (5 * 1024 * 1024)) {
             showNotification("File gambar terlalu besar (Max 5MB).", "error");
             return;
        }
        submitBtn.disabled = true; 
        submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Mengirim...'; 
        const url = `/api/submit-report`; 
        ...
        try { 
            ...
            const reportType = formData.get('reportType') || 'Laporan';
            showNotification(result.message || `${reportType} sent!`, 'success'); 
            ...
            showNotification(err.message || "Gagal mengirim laporan.", 'error'); 
        } finally { 
            submitBtn.disabled = false; 
            submitBtn.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Kirim'; 
        } 
    }); 
}
    ...
    const sidebarLinks = document.querySelectorAll(".sidebar-nav a:not(#sidebarAboutBtn)");
    ...
    if (sidebarAboutBtn) { sidebarAboutBtn.addEventListener("click", (e) => { e.preventDefault(); closeSidebar(); openModal("aboutApiModal"); }); }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const mainApiPage = document.getElementById('main-api-page');
    const preloader = document.getElementById('preloader'); 
    const docsButton = document.getElementById('docs-page-button');
    const body = document.body;

    function showDocsPage() {
        if (mainApiPage) mainApiPage.style.display = 'none';
        if (preloader) preloader.classList.add('hidden');
        if (docsPage) {
            docsPage.style.display = 'flex';
            setTimeout(() => docsPage.classList.add('visible'), 50); 
        }
        body.setAttribute("data-theme", "dark");
        body.style.backgroundImage = 'none';
        body.style.background = 'var(--darker)';
    }

    async function showApiPage() {
        if (docsPage) {
            docsPage.classList.remove('visible');
            setTimeout(() => docsPage.style.display = 'none', 500);
        }
        if (mainApiPage) mainApiPage.style.display = 'block';
        if (preloader) preloader.classList.remove('hidden');
        
        await initializeMainApiPage();
    }
    body.setAttribute("data-theme", "dark");
    body.style.background = 'var(--darker)'; 
    updateDateTime();
    setInterval(updateDateTime, 1000);
    if (docsButton) docsButton.addEventListener('click', () => {
        window.location.hash = 'docs';
    });
    const sidebarHomeLink = document.querySelector('.sidebar-nav a[href="#"]');
    if (sidebarHomeLink) {
    ...
    document.addEventListener("click", (e) => {
        ...
        const modalClicked = e.target.classList.contains('modal') ? e.target : null; 
        const activeModal = closeBtn ? closeBtn.closest('.modal.active') : (modalClicked && modalClicked.classList.contains('active') ? modalClicked : null); 
        
        if (activeModal) { 
            if (closeBtn || (modalClicked && !modalClicked.querySelector('.modal-dialog').contains(e.target))) { 
                if (activeModal.id === 'donationModal') {
                    if (closeBtn) activeModal.classList.remove('active'); 
                } else {
                    activeModal.classList.remove('active');
                }
                if (isApiInitialized && activeModal.id === 'apiResponseModal') { 
                    const rEl=document.getElementById("apiResponseContent"); if(rEl){rEl.classList.add("d-none"); rEl.innerHTML = ""; rEl.className='d-none';} 
                    const fEl=document.getElementById("paramForm"); if(fEl) fEl.style.display = "block"; 
                    const sBtn=document.getElementById("submitParamBtn"); if(sBtn) sBtn.style.display = "none"; 
                    const cpS=activeModal.querySelector(".copy-section"); if(cpS) cpS.classList.remove("active"); 
                    const cpR=document.getElementById("copyResponseBtn"); if(cpR) cpR.classList.add("d-none"); 
                    const ld=document.getElementById("apiResponseLoading"); if(ld) ld.style.display = "none"; 
                    const rt=document.getElementById("retryRequestBtn"); if(rt) rt.classList.add('d-none'); 
                    lastFetchedUrl = ""; 
                } 
                if (activeModal.id === 'reportModal') { 
                    const rF=document.getElementById("reportForm"); if(rF) rF.reset(); 
                    const sRB=document.getElementById("submitReportBtn"); if(sRB) { sRB.disabled = false; sRB.innerHTML = '<i class="fa-solid fa-paper-plane"></i> Kirim';} 
                } 
            } 
        }
    });
    ...
    setTimeout(() => {
        if (initialLoader) {
            ...
            setTimeout(() => initialLoader.style.display = 'none', 1000);
        }
        handleRouting();
    }, 3000);
